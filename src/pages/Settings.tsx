      ...
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center gap-3 mb-8">
          <div className="w-10 h-10 rounded-xl bg-[#ccff00]/10 flex items-center justify-center">
            <SettingsIcon className="w-5 h-5 text-[#ccff00]" />
          </div>
          <div>
            <h1 className="text-xl font-bold text-white">系统设置</h1>
            <p className="text-xs text-[#666]">API 配置与系统状态</p>
          </div>
        </div>

        <div className="rounded-2xl bg-[#111] border border-white/[0.05] p-8 text-center">
          <Shield className="w-10 h-10 text-[#ccff00]/40 mx-auto mb-3" />
          <h3 className="text-sm font-bold text-white mb-1">需要管理员权限</h3>
        ...
        <div className="mt-6 space-y-3">
          ...
          {platformList.map((p) => (
            <div
              key={p.key}
              className="flex items-center justify-between rounded-xl bg-[#111] border border-white/[0.05] p-4"
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-[#666]">{p.description}</p>
                </div>
              </div>
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="text-xl font-bold text-white">系统设置</h1>
          <p className="text-xs text-[#666]">配置 RapidAPI Key 以启用自动数据获取</p>
        </div>
      </div>

      {/* Test Mode Toggle — visible to all authenticated users */}
      <div className="mb-6 rounded-xl bg-white/[0.02] border border-white/[0.06] p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <FlaskConical className={`w-5 h-5 ${testModeEnabled ? "text-[#ccff00]" : "text-[#555]"}`} />
            <div>
              <h3 className="text-sm font-bold text-white">开发测试模式</h3>
          ...
          <button
            ...
            className={`relative w-11 h-6 rounded-full transition-colors ${
              testModeEnabled ? "bg-[#ccff00]" : "bg-white/[0.08]"
            }`}
          >
            <div
        ...
        {testModeEnabled && (
          <div className="mt-3 pt-3 border-t border-white/[0.04] flex items-center gap-2">
            <span className="text-[10px] px-2 py-0.5 rounded-md bg-[#ccff00]/10 text-[#ccff00] font-medium">测试模式运行中</span>
            {isAdmin && (
              <button
                onClick={() => {
        ...
        <div>
          <p className="text-xs text-[#aaa] leading-relaxed">
            ...
            <a
              ...
              className="text-[#ccff00] hover:underline inline-flex items-center gap-0.5"
            >
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">邀请码管理</h1>
            <p className="text-xs text-[#666]">生成和管理注册邀请码</p>
          </div>
        </div>

        {/* Generate codes section */}
        <div className="rounded-xl bg-[#111] border border-white/[0.05] p-4 mb-4">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              <span className="text-xs text-[#888]">生成数量:</span>
              <input
                type="number"
                min={1}
                max={50}
                ...
                className="w-16 bg-[#0a0a0a] border border-white/[0.08] rounded-lg px-2 py-1.5 text-xs text-white text-center focus:outline-none focus:border-[#ccff00]/30"
              />
            </div>
            <button
            ...
            {generateCodes.isSuccess && generateCodes.data && (
              <div className="flex items-center gap-2 flex-wrap">
                {generateCodes.data.codes.map((code) => (
                  <span
                    key={code}
                    className="inline-flex items-center gap-1 text-xs px-2 py-1 rounded-md bg-[#ccff00]/10 text-[#ccff00] font-mono font-bold"
                  >
                    {code}
                    <button
        ...
        {invitationList && invitationList.length > 0 ? (
          <div className="rounded-xl bg-[#111] border border-white/[0.05] overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-white/[0.05]">
                    <th className="text-left text-[10px] text-[#666] font-medium px-4 py-2.5">邀请码</th>
                    <th className="text-left text-[10px] text-[#666] font-medium px-4 py-2.5">状态</th>
                    <th className="text-left text-[10px] text-[#666] font-medium px-4 py-2.5">使用者</th>
                    <th className="text-left text-[10px] text-[#666] font-medium px-4 py-2.5">使用时间</th>
                    <th className="text-left text-[10px] text-[#666] font-medium px-4 py-2.5">生成时间</th>
                    <th className="text-right text-[10px] text-[#666] font-medium px-4 py-2.5">操作</th>
                  </tr>
                </thead>
                <tbody>
                  {invitationList.map((inv) => (
                    <tr key={inv.id} className="border-b border-white/[0.03] last:border-0 hover:bg-white/[0.01] transition-colors">
                      <td className="px-4 py-2.5">
                        <span className="text-xs font-mono font-bold text-[#ccff00]">{inv.code}</span>
                      </td>
                      <td className="px-4 py-2.5">
                        {inv.usedByUnionId ? (
                          <span className="inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded-full bg-white/[0.03] text-[#666]">
                      ...
                      <td className="px-4 py-2.5">
                        <span className="text-[11px] text-[#666]">{inv.usedAt || "-"}</span>
                      </td>
                      <td className="px-4 py-2.5">
                        <span className="text-[11px] text-[#666]">{inv.createdAt}</span>
                      </td>
          ...
          <div className="rounded-xl bg-[#111] border border-white/[0.05] p-8 text-center">
            <Ticket className="w-8 h-8 text-[#444] mx-auto mb-2" />
            <p className="text-xs text-[#666]">暂无邀请码，点击上方按钮生成</p>
          </div>
        )}
      </div>
          ...
          return (
            <div
              ...
              className={`rounded-xl border transition-all ${
                isExpanded
                  ? "bg-[#111] border-[#ccff00]/20"
                  : "bg-[#111] border-white/[0.05] hover:border-white/[0.1]"
              }`}
            >
              {/* Card header */}
              <div
                    ...
                    <button
                      ...
                      className={`px-3 py-1 rounded-full text-[10px] font-medium transition-all ${
                        cfg.isActive
                          ...
                          : "bg-white/[0.03] text-[#666] hover:bg-white/[0.06]"
                      }`}
                    >
                ...
                <div className="border-t border-white/[0.05] p-4 space-y-4">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full bg-[#0a0a0a] border border-white/[0.08] rounded-lg px-3 py-2.5 pr-10 text-xs text-white placeholder:text-[#444] focus:outline-none focus:border-[#ccff00]/30 transition-colors"
                      />
                      <button
                        onClick={() =>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-[#0a0a0a] border border-white/[0.08] rounded-lg px-3 py-2.5 text-xs text-white placeholder:text-[#444] focus:outline-none focus:border-[#ccff00]/30 transition-colors"
                    />
                  ...
                  <div className="flex items-center justify-between pt-2">
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        ...
                        className="text-xs text-[#666] hover:text-white px-3 py-2 rounded-full hover:bg-white/[0.03] transition-all"
                      >
                        取消
                      </button>
                    </div>
